...
def cifar10_get_loaders(dir_, batch_size, distributed=False):
...
def attack_pgd(device,model, X, y, epsilon, alpha, attack_iters, restarts):
...
# def evaluate_pgd(device,test_loader, model, attack_iters, restarts, epsilon=(8 / 255.) / std):
#     alpha = (2 / 255.) / std
#     pgd_loss = 0
#     pgd_acc = 0
#     n = 0
...
def evaluate_pgd(device,test_loader, model, clean_preds, true_labels, attack_iters, restarts, epsilon):
    alpha = (2 / 255.) / std
    pgd_loss = 0
    pgd_acc = 0
    n = 0
...
def attack_fgsm(device,model, X, y, epsilon, alpha, restarts):
    attack_iters = 1
    max_loss = torch.zeros(y.shape[0]).to(device)
    max_delta = torch.zeros_like(X).to(device)
    for zz in range(restarts):
...
def evaluate_fgsm(device,test_loader, model, restarts):
    epsilon = (8 / 255.) / std
    alpha = (8 / 255.) / std
    pgd_loss = 0
    pgd_acc = 0
    ...
    for i, (X, y) in enumerate(test_loader):
        X, y = X.to(device), y.to(device)
        pgd_delta = attack_fgsm(device,model, X, y, epsilon, alpha, restarts)
        with torch.no_grad():
...
def cw_Linf_attack(device,model, X, y, epsilon, alpha, attack_iters, restarts):
    max_loss = torch.zeros(y.shape[0]).to(device)
    max_delta = torch.zeros_like(X).to(device)
    for zz in range(restarts):
        delta = torch.zeros_like(X).to(device)
...
def evaluate_pgd_cw(device,test_loader, model, attack_iters, restarts):
    alpha = (2 / 255.) / std
    epsilon = (8 / 255.) / std
    pgd_loss = 0
    pgd_acc = 0
    ...
    for i, (X, y) in enumerate(test_loader):
        X, y = X.to(device), y.to(device)
        pgd_delta = cw_Linf_attack(device,model, X, y, epsilon, alpha, attack_iters=attack_iters, restarts=restarts)
        with torch.no_grad():
            output = model(X + pgd_delta)
            loss = F.cross_entropy(output, y)
            pgd_loss += loss.item() * y.size(0)
...
def get_variable(device,inputs, cuda=False, **kwargs):
...
def evaluate_autoattack(device,test_loader, model, epsilon=8/255., norm='Linf', attacks_to_run=['apgd-ce', 'apgd-dlr',  'square']):
    model.eval()
    ...
    for X, y in test_loader:
        all_inputs.append(X)
        all_labels.append(y)
    X_test = torch.cat(all_inputs, dim=0).to(device)
